...
-- Create a query to get the all rows in the comments table, showing post title (aliased as 'Post Title'), and the all the comment's fields (19729 rows)

SELECT comments.*, posts.title AS "Post Title"
FROM comments
JOIN posts ON (comments.posts_id = posts.id);

-- Create a query to get the all rows in the comments table, showing post title (aliased as post_title), post url (ailased as post_url), and the comment body (aliased as comment_body) where the post was created before January 1, 2015 (9126 rows)

SELECT posts.title AS post_title, posts.url AS post_url, comments.body AS comment_body
FROM comments JOIN posts ON (comments.posts_id = posts.id)
WHERE posts.created_at <= '2015-01-01 00:00:00.000';
...
-- Create a query to get the all rows in the comments table, showing post title (aliased as post_title), post url (ailased as post_url), and the comment body (aliased as comment_body) where the comment body contains the word 'USB' (728 rows)
...
SELECT posts.title AS post_title, users.first_name AS first_name, users.last_name AS last_name, comments.body AS comment_body
...
SELECT users.first_name AS first_name, users.last_name AS last_name, comments.body AS comment_body
FROM posts JOIN users ON (posts.users_id = users.id)
 JOIN comments ON (comments.posts_id = posts.id)
